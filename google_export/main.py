...
def get_awards_by_id(user_id: str | int, key: str, curl_args, timeout, rec_limit=10) -> dict:
    ...
    try:
        if not (not isinstance(user_id, str) or user_id.isdigit()):
            user_id = get_id_by_name(user_id, curl_args)

        c = httpx.get(
            f'https://developerprofiles-pa.clients6.google.com/v1/awards?access_token&locale&obfuscatedProfileId={user_id}&useBadges=true&key={key}',
            headers={
                ...
                'X-ClientDetails': '''appVersion=5.0%20(X11)&platform=Linux%20x86_64&userAgent=Mozilla%2F5.0%20(X11%3B%20Linux%20x86_64%3B%20rv%3A128.0)%20Gecko%2F20100101%20Firefox%2F128.0''',  # noqa: E501
        ...
        award_titles = {
            ...
            for award in data.get(
                'awards', {}
            )  # this line handles bad ids (if err -> no rewards -> 0 on public profile column)
        }
        return award_titles
    except httpx.ConnectError:
        if rec_limit == 0:
...
def write_to_local_csv(awards: dict[set], curl_args, fname: str = 'result.csv', rec_limit=10) -> None:
    ...
    with open(fname, 'w', newline='') as csvfile:
        ...
        award_writer.writerow(column_names)
        rows = []
...
if __name__ == '__main__':
    ...
    # ids = lines
        ...
        lines = sheets.read_ids_from_table(
            args.google_token, args.table_id, args.input_sheet_id, args.input_column_number
        )
    ...
    if args.google_token and args.table_id and args.sheet_id:
        sheets.write_data_to_table(pandas.read_csv(args.output), args.google_token, args.table_id, args.sheet_id)

    if args.yandex_token and args.yandex_path:
        import yandex_disk

        yandex_disk.DiskManager(yatoken=args.yandex_token).upload(args.output, args.yandex_path)
