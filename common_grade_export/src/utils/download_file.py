...
def download_sheets(
    ...
    try:
        ...
        if not content:
            logger.error("Ошибка экспорта файла")
            return None
...
def merge_multiple_pdfs(table_id: str, sheet_ids: list[str], access_token: str) -> bytes:
    ...
    """
    merger = PdfMerger()
    for _, sheet_id in enumerate(sheet_ids):
...
def export_file(table_id: str, sheet_id: str, access_token: str, export_format: str) -> bytes | None:
    """
...
def parse_args():
    ...
    parser.add_argument(
        ...
        help="Sheet IDs separated by ; (default: 0)",
    )
    parser.add_argument("--format", choices=["csv", "pdf", "xlsx"], default="csv", help="Output format")
    parser.add_argument("--filename", default="export", help="Output filename (without extension)")
